...
def add_unique(cases: list[UnaryCase], seen: set[int], label: str, fmt: ZkfFormat, x: int) -> None:
    ...
    if key in seen:
        return
    seen.add(key)
    cases.append(UnaryCase(label, x, fmt.wrap(x).saturate().bits))
...
@cocotb.test()
async def saturate_runtime_cases(dut) -> None:
    context = float_context("saturate")
    ...
    for index, case in enumerate(cases):
        ...
        assert observed == case.expected, (
            ...
            f"expected={hex_bits(case.expected, fmt.wfull)} observed={hex_bits(observed, fmt.wfull)}"
        )
